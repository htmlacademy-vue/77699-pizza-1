...
describe("PizzaFoundation", () => {
  ...
  it("renders a pizzaContent class", () => {
    ...
    expect(wrapper.find("[data-test='pizzaContent']").classes()).toContain("pizza--foundation--small-creamy");
  });
  ...
  it("renders a pizzaFilling class with ingredients", () => {
	getData();
    createComponent({ localVue, store });
    expect(wrapper.find("[data-test='pizzaFilling']").classes()).toContain("pizza__filling--ham");
    expect(wrapper.find("[data-test='pizzaFilling']").classes()).toContain("pizza__filling--second");
  });
});
