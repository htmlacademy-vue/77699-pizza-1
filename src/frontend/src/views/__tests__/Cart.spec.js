...
describe("Cart", () => {
  ...
  const getData = () => {
    ...
    store.commit("Profile/GET_ADDRESSES", [{ id: 1, name: "testName", street: "testStreet", building: "testBuilding", flat: "testFlat", comment: "testComment" }]);
  ...
  it("renders disabled button", () => {
    createComponent({ localVue, store, stubs });
    expect(wrapper.find("[data-test='addOrder']").attributes("disabled")).toBeTruthy();
  });
  
  it("renders enabled button", () => {
	getData();
    createComponent({ localVue, store, stubs });
    expect(wrapper.find("[data-test='addOrder']").attributes("disabled")).toBeFalsy();
  });
  ...
  it("raises the addOrder event on button click", async () => {
    ...
    expect(actions.Profile.postOrder).toHaveBeenCalledWith(
      ...
      { userId: "76841285-f38b-4c88-9929-0ad073c64731", pizzas: [{ name: "testName", orderId: 1, quantity: 1, sauceId: 1, sizeId: 2, doughId: 1, id: 1, ingredients: [{id: 2, quantity: 2, pizzaId: 2, ingredientId: 9}] }], misc: [{id: 3, quantity: 1, orderId: 6, miscId: 2}], address: null }
	);
  });
});
