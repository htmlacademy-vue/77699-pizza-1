...
describe("Orders", () => {
  ...
  const getData = () => {
    ...
    store.commit("Builder/CHANGE_FILLINGS", { filling: "ham", count: 2, id: 8, fillingPrice: 42 });
  ...
  it("renders pizza sauce", () => {
    getData();
	createComponent({ localVue, store });
    expect(wrapper.find("[data-test='pizzaSauce']").text()).toContain("томатный");
  });
  
  it("renders pizza fillings", () => {
    getData();
	createComponent({ localVue, store });
    expect(wrapper.find("[data-test='pizzaFillings']").text()).toContain("ветчина");
  });  
