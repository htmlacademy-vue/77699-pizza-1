...
describe("BuilderSizeSelector", () => {
  ...
  it("RadioButton labelClass contains size.value", () => {
	getData();
    createComponent({ localVue, store, listeners });
    expect(wrapper.findAllComponents(RadioButton).at(1).props("labelClass")).toContain("normal");
  });
  
  it("RadioButton value is size.value", () => {
	getData();
    createComponent({ localVue, store, listeners });
    expect(wrapper.findAllComponents(RadioButton).at(1).props("value")).toEqual("normal");
  });
  
  it("RadioButton checked if size.id == sizeId", () => {
	getData();
    createComponent({ localVue, store, listeners });
    expect(wrapper.findAllComponents(RadioButton).at(1).props("checked")).toBeTruthy();
  });
