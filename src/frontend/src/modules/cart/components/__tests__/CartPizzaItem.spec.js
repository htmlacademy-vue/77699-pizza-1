...
describe("CartPizzaItem", () => {
  ...
  const propsData = {
    pizza: {
      ...
      sauceId: 1,
      count: 2,
      ingredients: [{ id: 8, value: "ham", name: "Ветчина", price: 42 }],
      price: 868,
      name: "testName",
      indx: -1,
    },
  ...
  const getData = () => {
    ...
    store.commit("Cart/CHANGE_CART_PIZZA", {
      pizza: propsData.pizza,
      indx: -1,
    });
  };
  ...
  it("renders pizza name", () => {
    ...
    expect(wrapper.find("[data-test='pizzaName']").text()).toBe(
      propsData.pizza.name
    );
  });
  ...
  it("PizzaItemCounter counterValue is pizza.count", () => {
    ...
    createComponent({ localVue, store, propsData, listeners });
    expect(
      wrapper.findAllComponents(PizzaItemCounter).at(0).props("counterValue")
    ).toEqual(propsData.pizza.count);
  });
  ...
  it("PizzaItemCounter pizzaPrice is pizza.price", () => {
    ...
    createComponent({ localVue, store, propsData, listeners });
    expect(
      wrapper.findAllComponents(PizzaItemCounter).at(0).props("pizzaPrice")
    ).toEqual(propsData.pizza.price);
  });
  ...
  it("calls change count event", async () => {
    ...
    await wrapper
      .findAllComponents(PizzaItemCounter)
      .at(0)
