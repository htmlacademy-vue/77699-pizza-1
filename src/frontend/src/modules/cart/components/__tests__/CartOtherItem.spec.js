...
describe("CartOtherItem", () => {
  ...
  it("CartItemCounter itemPrice is item.price", () => {
    createComponent({ localVue, store, propsData });
    expect(
      wrapper.findAllComponents(CartItemCounter).at(0).props("itemPrice")
    ).toEqual(propsData.item.price);
  });

  it("CartItemCounter id is item.id", () => {
    createComponent({ localVue, store, propsData });
    expect(
      wrapper.findAllComponents(CartItemCounter).at(0).props("id")
    ).toEqual(propsData.item.id);
  });
});
