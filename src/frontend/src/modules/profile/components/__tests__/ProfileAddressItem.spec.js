...
describe("ProfileAddressItem", () => {
  ...
  it("renders ProfileNewAddress properties", () => {
    ...
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("street")
    ).toEqual(propsData.address.street);
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("name")
    ).toEqual(propsData.address.name);
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("building")
    ).toEqual(propsData.address.building);
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("flat")
    ).toEqual(propsData.address.flat);
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("comment")
    ).toEqual(propsData.address.comment);
    expect(
      wrapper.findAllComponents(ProfileNewAddress).at(0).props("id")
    ).toEqual(propsData.address.id);
  });
});
